"""
Converted project pipeline for Graha-Sūcana / ChandraQuant.

This file was generated from the final code cell of the uploaded notebook.
The model/data/evaluation logic is intentionally preserved; the only project
formatting changes are:
- notebook-time pip installs were moved to requirements.txt / pyproject.toml
- code was wrapped in run_experiment() so importing the package does not run it
- date handling in compute_astro() was made robust across pandas datetime types
"""
...
def run_experiment():
    ...
    if raw.empty:
        raise RuntimeError(
            ...
            "Check your internet connection, ticker availability, and Yahoo Finance access."
        )
    ...
    if not frames:
        raise RuntimeError(
            "No usable close-price series were found after downloading market data."
        )
    ...
    def compute_astro(dates_list):
        """Compute astronomical features for a list of dates.

        This keeps the notebook's feature logic unchanged while making the date
        handling safe for pandas Timestamp, Python datetime, and numpy datetime64
        values returned by different pandas versions.
        """
        date_index = pd.DatetimeIndex(pd.to_datetime(dates_list)).tz_localize(None)
        aware = [d.to_pydatetime().replace(tzinfo=timezone.utc) for d in date_index]
    ...
    unique_dates = sorted(pd.to_datetime(market["Date"].unique()))
